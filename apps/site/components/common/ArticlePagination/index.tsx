...
export const ArticlePagination: FC<ArticlePaginationProps> = ({
  ...
  const baseClass = `
        ...
        focus:outline-hidden focus:ring-1 focus:ring-lime-600/60 
    `;
  const labelCalss = `
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between">
        ...
        {next && (
          <div className="group flex-1 sm:text-right">
            <Link
              href={next.href}
              className={cn(baseClass, `block justify-end`)}
              aria-label={`下一页：${next.title}`}
            >
              <div className="flex flex-col">
                <div className="flex flex-row items-center justify-center">
                  <span className="flex-1 text-xs/5 text-gray-500">
                    Next Page
                  </span>
