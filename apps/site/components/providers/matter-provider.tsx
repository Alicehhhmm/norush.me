...
import { assignClientContext } from '@/apps/site/utils/assignClientContext'

/**
 * 创建全局共享的内容上下文
 * 使用默认值初始化，避免服务端数据未传入时的空值问题
 */
export const MatterContext = createContext<ClientSharedServerContext>(assignClientContext({}))
...
export const MatterProvider: FC<MatterProviderProps> = ({ children, ...data }) => {
    ...
    return <MatterContext.Provider value={contextValue}>{children}</MatterContext.Provider>
}
