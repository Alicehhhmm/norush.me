...
const fetchMatchedCollections = async (
  ...
  const parentId =
    allCollections &&
    allCollections.find(c => c._id === DEFAULT_RAINDROP_COLLECTIONS_ID)?._id;

  const matched = (subCollections ?? []).filter(
    c =>
      c.parent?.$id === parentId && siteChannels.some(sc => sc.key === c.slug)
  );
