...
export function useThemeManager() {
  ...
  const parsedTheme = useMemo((): ThemeParts => {
    ...
    if (parts.length === 2) {
      ...
      const colorTheme = COLOR_THEMES.includes(colorStr as ColorTheme)
        ? (colorStr as ColorTheme)
        : 'default';
      const mode = MODES.includes(modeStr as Mode)
        ? (modeStr as Mode)
        : 'system';
    ...
    const fallback = MODES.includes(theme as Mode)
      ...
      : {
          colorTheme: COLOR_THEMES.includes(theme as ColorTheme)
            ? (theme as ColorTheme)
            : 'default',
